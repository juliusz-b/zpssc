...
OPT_LW   = 3.0           # optical line width
EL_LW    = 1.4           # electronic arrow width
...
def box(x, y, w, h, text, fc=C_ELEC, ec=INK, lw=1.4, fs=8.0, ls="solid",
...
ax.text((b_bt["r"] + b_vcsel["l"]) / 2, b_vcsel["cy"] + 0.62,
        "$I_\\mathrm{LD}(t)$", fontsize=7, color=C_ELECLN,
        ha="center", va="bottom")
...
ax.text(b_vcsel["cx"] + 0.15, (b_tune["t"] + b_vcsel["b"]) / 2,
        "$\\lambda$ sweep", fontsize=6.8, color=C_ELECLN, ha="left",
        va="center")
...
ax.text(b_circ["cx"], b_circ["t"] - 0.4, "circulator", fontsize=8,
        ha="center", va="center", color="black")
ax.text(b_circ["l"] + 0.3, b_circ["cy"] + 0.35, "1", fontsize=7.5,
        color=INK, ha="center", va="center", weight="bold")
ax.text(b_circ["r"] - 0.3, b_circ["cy"] + 0.05, "2", fontsize=7.5,
        color=INK, ha="center", va="center", weight="bold")
ax.text(b_circ["l"] + 0.3, b_circ["b"] + 0.4, "3", fontsize=7.5,
        color=INK, ha="center", va="center", weight="bold")
# curved 1->2->3 hint inside
ax.add_patch(FancyArrowPatch((b_circ["cx"] - 0.55, b_circ["cy"] + 0.3),
                             (b_circ["cx"] + 0.5, b_circ["cy"] - 0.1),
                             arrowstyle="-|>", mutation_scale=8,
                             lw=1.0, color=C_OPTLN,
...
ax.add_patch(Circle((tap_x, tap_y), 0.16, fc="white", ec=C_OPTLN,
                    lw=2.0, zorder=5))
ax.text(tap_x, tap_y + 0.34, "90/10\ncoupler", fontsize=6.4, color=C_OPTLN,
        ha="center", va="bottom")
...
ax.text((18.4 + rail_end) / 2, ret_y - 0.32, "reflections (sum of channels)",
        fontsize=6.8, color=C_OPTLN, ha="center", va="top")
...
for gx, lab, cc in zip(fbg_x, fbg_lab, fbg_codes):
    ax.add_patch(Rectangle((gx - 0.28, chain_y - 0.42), 0.56, 0.84,
                           fc="#f4c27a", ec=INK, lw=1.2, zorder=4))
    for fxoff in np.linspace(-0.18, 0.18, 5):
        ax.plot([gx + fxoff, gx + fxoff],
                [chain_y - 0.36, chain_y + 0.36],
                color=INK, lw=0.7, zorder=5)
    ax.text(gx, chain_y + 0.52, lab, fontsize=7.0, ha="center",
            va="bottom", color="black")
    ax.text(gx, chain_y - 0.52, cc, fontsize=7.2, ha="center", va="top",
            color=INK)
...
ax.text((fbg_x[0] + fbg_x[1]) / 2, chain_y + 1.62, "$\\approx$4 m",
        fontsize=6.8, color="#666", ha="center", va="bottom")
...
ax.text((fbg_x[0] + fbg_x[-1]) / 2, chain_y + 2.05,
        ...
        fontsize=7.0, ha="center", va="bottom", color="black")
...
ax.text(b_pel["cx"], b_pel["t"] - 0.3, "Peltier stage (per grating)",
        fontsize=7.2, ha="center", va="center", weight="bold")
...
ax.text(b_circ["cx"] - 0.6, (b_circ["b"] + 1.45) / 2 + 1.1,
        "2$\\rightarrow$3", fontsize=6.8, color=C_OPTLN, ha="right",
        va="center")
...
b_mzi = box(11.0, 3.35, 3.0, 1.6,
            "unbalanced MZI\n($\\Delta L$ k-clock)\nFSR $\\approx$ 10 GHz",
            fc=C_OPT, ec=C_GREY, lw=1.3, ls="dashed", fs=7.4, tcol="#444")
b_pdref = box(14.7, 3.5, 1.9, 1.3, "PD$_\\mathrm{ref}$\n+ ADC",
              fc=C_PROC, ec=C_GREY, lw=1.3, ls="dashed", fs=7.4, tcol="#444")
...
oline([(tap_x, tap_y - 0.16), (tap_x, 6.7), (11.0 + 0.9, 6.7),
       (11.0 + 0.9, b_mzi["t"])], color=C_GREY, ls="dashed", lw=2.2,
      arrow=True, head=12)
ax.text(tap_x + 0.2, 7.1, "10%", fontsize=6.6, color=C_GREY, ha="left",
        va="center")
# MZI -> PD_ref (optical, dashed) -> ruler (electronic, dashed)
oline([(b_mzi["r"], b_mzi["cy"]), (b_pdref["l"], b_pdref["cy"])],
      color=C_GREY, ls="dashed", lw=2.2, arrow=True, head=12)
...
earrow((b_ruler["l"], b_ruler["t"] - 0.15), (b_proc["r"] - 0.4, b_proc["b"]),
       color=C_GREY, ls="dashed", lw=1.0, rad=-0.2, head=9)
ax.text(11.0, 5.15, "optional axis-linearization branch", fontsize=7.0,
...
legend_elems = [
    ...
    Line2D([0], [0], color=C_ELECLN, lw=EL_LW, marker=">", markersize=6,
           label="electronic signal"),
    Line2D([0], [0], color=C_GREY, lw=2.0, ls="dashed",
           label="optional MZI k-clock branch"),
    Line2D([0], [0], color=C_THERM, lw=1.2, ls="dashed",
           label="thermal (Peltier/PID) control"),
]
leg = ax.legend(handles=legend_elems, loc="lower left",
                bbox_to_anchor=(0.005, 0.18), fontsize=7.4,
