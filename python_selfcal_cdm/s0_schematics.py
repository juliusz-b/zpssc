"""Schematic figures: (Fig.1) system block diagram, (Fig.2) wavelength-time
mapping for the coded sweep."""
...
box(0.1,1.1,1.8,0.9,"Tunable\nVCSEL 1550 nm\n+ code (intensity mod.)")
box(2.3,1.2,1.0,0.7,"Circulator")
box(3.7,0.5,3.2,2.0,"FBG chain\n S1  R1  S2  R2  S3\n(sensors + references\non Peltier stages)",fc='#fdf3e7')
box(7.3,1.2,1.0,0.7,"APD\nInGaAs")
box(8.7,0.9,2.1,1.3,"STM32H7:\ndespread (FFT)\n+ reference calib.\n+ wavelength extraction",fc='#eafaf0')
...
ax.text(5.3,2.62,"Fig. 1. Block diagram: code-driven tunable VCSEL -> FBG chain (sensors + references) -> APD -> despreading and self-calibration on the edge node.",ha='center',fontsize=7.5)
...
ax.plot(t,nu,'b',lw=1.6,label='VCSEL sweep $\\nu_s(t)$')
nu_nl=2.0*t-1.0+0.18*np.sin(2*np.pi*t)  # nonlinear
ax.plot(t,nu_nl,'b--',lw=1.0,alpha=0.7,label='actual (nonlinear)')
...
ax.plot(tt,code,'k',lw=0.7); ax.text(0.1,0.99,'code c(t)',fontsize=7,ha='center')
ax.set_xlabel('sweep time (norm.)'); ax.set_ylabel('optical detuning (norm.)')
ax.set_title('Fig. 2. Wavelength-time mapping: each grating reflects when $\\nu_s(t)$ crosses its resonance;\nthe code provides despreading gain, while sweep nonlinearity and chirp require calibration.',fontsize=7.5)
