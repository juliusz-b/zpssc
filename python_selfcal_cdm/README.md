# python_selfcal_cdm - samokalibrujący się układ przesłuchujący CDM (badanie symulacyjne)

Lekkie, odtwarzalne symulacje w Pythonie do artykułu JCR (EN) o samokalibrującym
się układzie przesłuchującym CDM (interrogatorze) sieci siatek Bragga z tanim, bezpośrednio
...
| Plik | Opis |
|------|------|
| `common.py` | wspólna fizyka + parametry sprzętu: kody Gold/Kasami, widma FBG (Gauss/tanh²), model świergotu lasera (chirp) i konwersji FM→AM, estymatory piku (dopasowanie Gaussa / środek ciężkości), nieliniowa charakterystyka przestrajania VCSEL, linijka MZI |
| `s0_schematics.py` | schematy poglądowe: diagram blokowy + odwzorowanie długość fali-czas |
| `s1_codes.py` | właściwości korelacyjne kodów; poziom listków vs długość kodu (Fig.3) |
| `s2_despread.py` | korelacyjne rozdzielenie (despreading) nakładających się siatek + kanały referencyjne (Fig.4, waterfall) |
| `s3_residual.py` | rezyduum błędu λ_B po korekcie referencjami vs świergot/FWHM i niedopasowanie nachylenia (Fig.5) |
...
| `s8_noise.py` | poziom szumów: jitter środka ciężkości vs SNR i reflektancja |
...
| `s12_capacity.py` | **pojemność sieci**: błąd λ_B vs liczba siatek z rozkładem na mechanizmy (przesłanianie widmowe, echa pozorne, przesłuch kodowy, szum); rozstaw równomierny vs losowy; sekwencyjna korekta przesłaniania; pojemność vs reflektancja |
...
| `s18_source.py` | świergot wywołany kodem (składowa adiabatyczna + przerzut), jądro p(δ), konwersja FM-AM na zboczu, rezyduum vs liczba referencji |
| `s19_deshadow.py` | przesłanianie widmowe i rekurencyjna korekta: schemat, linia przed/po, zysk i granica |
| `s20_inversion.py` | **czego korekta musi znać**: wariant bez założenia kształtu, tolerancja skali amplitudy, rozróżnianie echa pozornego od siatki po szerokości linii, dwie siatki w jednej komórce opóźnienia |
| `s21_cdmwdm.py` | **hybryda CDM-WDM**: adresowanie pasmo x komórka opóźnienia, cykl pomiarowy w czasie, dowód numeryczny że pasmowanie resetuje mechanizmy błędu (32 w 1 paśmie: 60,1 pm; 4x8: 22,1; 8 w 1 paśmie: 22,6) |
| `s22_codelength.py` | **długość kodu N jako zmienna projektowa**: komórki/zasięg, sufit przesłuchu 0,37N (potwierdzony dla N=63..511), czas ramki i odświeżanie, statystyka kolizji komórek (paradoks dnia urodzin) i oczekiwana liczba „trudnych par" ~0,26·K²/N |
| `s23_theory.py` | **teoria w zamkniętej formie, zweryfikowana**: prawo A (bias przesłaniania pary, dokładne do 1. rzędu w R, maks. 0,81·R·sigma przy odstrojeniu sigma·sqrt(3/2)), prawo B (średni bias wielodostępu, nieparzysty, liniowy w (K-1)/N, zdejmowany przez referencje 4x), nierówność C (K·f_r <= eps·f_s/(c_L·n_s·M), N się skraca) |
...
## Kluczowe wyniki
- Kody Gold N=127: korelacja wzajemna **0,134 = granica 17/127**; poziom listków maleje z długością kodu.
- Korelacyjne rozdzielenie CDM (idealny tor): MAE ≈ **9 pm**.
- Referencje usuwają wspólny offset świergotu → **poziom rezyduum ~kilka pm** (niedopasowanie nachylenia). Surowy błąd FM-AM **skaluje się ze świergotem** (świergot do zmierzenia na OSA; NIE zakładać konkretnej wartości).
...
- **Przesłanianie widmowe rządzi przy silnych siatkach**: przy R=10% już 3 siatki dają 8,1 pm, a 96 siatek 179 pm. Sekwencyjna korekta przesłaniania (peeling) sprowadza makietę do 0,6 pm i podnosi pojemność z 4 do 17 siatek przy R=10%.
- **Rozstaw siatek ma znaczenie strukturalne**: przy rozstawie równomiernym KAŻDE echo pozorne trzeciego rzędu mieszczące się w rozpiętości szeregu siatek trafia w zajętą komórkę opóźnienia; rozstaw losowy zostawia ok. 12%. Przy K=32 to różnica 45,0 vs 10,4 pm.
...
- **Korekta przesłaniania nie wymaga kształtu linii (s20)**: transmisję da się zbudować punkt po punkcie z samego pomiaru, `T = (1 - g·S)²`. Na siatkach tanh² wariant bezkształtowy daje 1,90 pm wobec 3,67 pm dla dopasowania gaussowskiego. Potrzebna jest tylko skala amplitudy i to z grubsza: korekta pomaga dla g od 0,5 do 1,6.
...
- **Dwie siatki w jednej komórce opóźnienia (s20)**: dopasowanie dwuskładnikowe rozdziela je od pół szerokości linii (9,2 pm), przy pełnej szerokości 0,55 pm. Poniżej pół szerokości zawodzi. To pozwala wymieniać szybkość chipową na separację widmową.
- **Budżet błędu (s15)**: makieta w zakupionej konfiguracji 9,9 pm (dominuje przesłanianie 8,1 i świergot 4,8), sieć zaprojektowana 3,3 pm. Trzy referencje tną rezyduum świergotu 24-krotnie względem braku referencji, czwarta daje już niewiele.
